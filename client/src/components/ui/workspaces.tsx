...
function Workspaces<T extends Workspace>({
  ...
  const selectedWorkspace = React.useMemo(() => {
    ...
    return (
      workspaces.find((workspace) => getWorkspaceId(workspace) === selectedWorkspaceId) ||
      workspaces[0]
    );
  }, [workspaces, selectedWorkspaceId, getWorkspaceId]);

  const handleWorkspaceSelect = React.useCallback((workspace: Workspace) => {
    onWorkspaceChange?.(workspace as T);
    setOpen(false);
  }, [onWorkspaceChange, setOpen]);

  const contextValue: WorkspaceContextValue = React.useMemo(() => ({
    open,
    setOpen,
    selectedWorkspace: selectedWorkspace as Workspace | undefined,
    workspaces: workspaces as Workspace[],
    onWorkspaceSelect: handleWorkspaceSelect,
    getWorkspaceId: getWorkspaceId as (workspace: Workspace) => string,
    getWorkspaceName: getWorkspaceName as (workspace: Workspace) => string,
  }), [
    open,
    setOpen,
    selectedWorkspace,
...
function WorkspaceContent({
  ...
  const filteredWorkspaces = React.useMemo(() => {
    ...
    return workspaces.filter((workspace) =>
      getWorkspaceName(workspace).toLowerCase().includes(searchQuery.toLowerCase()),
    );
  ...
  const defaultRenderWorkspace = (workspace: Workspace, isSelected: boolean) => (
    <div className="flex min-w-0 flex-1 items-center gap-2">
      ...
      <div className="flex min-w-0 flex-1 flex-col items-start">
        ...
        {typeof workspace.plan === "string" && workspace.plan.length > 0 && (
