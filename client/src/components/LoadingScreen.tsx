...
export default function LoadingScreen({ progress, fullscreen = true }: LoadingScreenProps) {
  ...
  // Burst: fire when progress crosses ~95% (end of cycle or controlled completion)
  useEffect(() => {
    if (safeProgress >= 95) {
      if (!hasBurst.current) {
      ...
      // Reset so it can fire again on next cycle (uncontrolled looping)
      ...
      >
        ...
        <motion.div animate={burstControls} className="relative h-28 w-28">
          ...
          <img
            ...
            className="h-28 w-28"
          />
          {/* J fill progress bar — centered over the ring */}
